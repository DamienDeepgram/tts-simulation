...
const runSimulation = async function () {
    ...
    for (let total_concurrent_calls = min_concurrent; total_concurrent_calls <= max_concurrent; total_concurrent_calls += inc_concurrent) {
        setTimeout(function() {
            status.innerHTML = "Running simulation for " + total_concurrent_calls + " concurrent calls";
        }, 0);

        // give some time to update the dom
        await new Promise(requestAnimationFrame);

        processNextStep(total_concurrent_calls, concurrency_limit, tts_time, turns_per_minute, steps);
    }
    plotResults(concurrentSteps, avgMaxs, avgCounts);
    setTimeout(function() {
        status.innerHTML = "Simulation Completed!";
    }, 0);
}

function processNextStep(total_concurrent_calls, concurrency_limit, tts_time, turns_per_minute, steps) {
    console.log('total_concurrent_calls', total_concurrent_calls);
    var fires = [];
